...
function initDOBSelectors() {
  ...
  if (!yEl || !mEl || !dEl) return; // 沒換 HTML 就略過
...
function buildProfileSummaryHTML() {
  ...
  const mingLine = mingTag
    ? `你的性格核心（命宮）：<span class="text-[#D4AF37] font-bold">${mingTag.tag}</span>（${toSafeText(mingPack.majors?.join("、") || "")}）`
    : `你的性格核心（命宮）：<span class="text-[#D4AF37] font-bold">${KB2026.empty_palace_copy.title}</span>（借對宮：${toSafeText(mingPack.opp?.name || "")}）`;

  const fudeLine = (fudePack.majors?.length)
    ? `快樂與安全感（福德）：${toSafeText(fudePack.majors.join("、"))}`
    : `快樂與安全感（福德）：${KB2026.empty_palace_copy.title}`;

  const jieLine = jieTag
    ? `壓力反應（疾厄）：${toSafeText(jiePack.majors?.join("、") || "")}`
    : `壓力反應（疾厄）：以「場景」與「紅綠燈」判讀更準`;

  const linkLine = `關係模式（夫妻 / 交友）：${toSafeText(fuqiPack.majors?.join("、") || "空宮")} ／ ${toSafeText(frPack.majors?.join("、") || "空宮")}`;
...
function initNavButtons() {
  const map = {
    "nav-chart": "section-chart",
    "nav-summary": "section-summary",
    "nav-annual": "section-annual",
    "nav-monthly": "section-monthly",
  };
  for (const [btnId, sectionId] of Object.entries(map)) {
